...
class BuildStandard:
    ...
    motor_hot_resistance_factor: float = 1.25   # Rm rises ~25% at temperature
...
def oswald_efficiency(ar: float, taper: float = 1.0) -> float:
    """Raymer straight-wing estimate, penalised for a rectangular planform."""
    e = 1.78 * (1.0 - 0.045 * ar ** 0.68) - 0.64
    if taper >= 0.999:
        e *= 0.96
    return min(max(e, 0.55), 0.92)
...
class AeroModel:
    ...
    def tail_cl(self, v: float, lift_n: float) -> float:
        ...
        qs = self.q(v) * max(self.s_tail_h, 1e-9)
    ...
    def drag(self, v: float, lift_n: float) -> float:
        ...
        if abs(lt) > 1e-9 and self.b_tail > 1e-6:
            d += lt * lt / (q * math.pi * self.b_tail ** 2
    ...
    def __init__(self, design: Design, cfg: Config):
        ...
        for _ in range(3):
            mb, empty, mtow_m2 = converge_empty_mass(
                ...
                b, n_ult)
            if n_override:
                self.load_case = design_load_factors(
                    self.aero_clean, mtow_m2 * G0,
                    self._max_level_speed(mtow_m2 * G0), b)
                n_ult = n_override
                break
            v_max = self._max_level_speed(mtow_m2 * G0)
            self.load_case: LoadCase = design_load_factors(
                self.aero_clean, mtow_m2 * G0, v_max, b)
            if abs(self.load_case.n_ultimate - n_ult) < 1e-4:
                n_ult = self.load_case.n_ultimate
                break
            n_ult = self.load_case.n_ultimate
        mb, empty, mtow_m2 = converge_empty_mass(
            self.wing, self.body, propulsion_items, self.m2_payload_kg,
...
def _make_design(base: Dict[str, Any], airfoil: str, x: Sequence[float],
    ...
    """x = (span, aspect ratio, sensor mass, simulator mass)."""
    span = max(x[0], 0.3)
...
def print_design_report(design: Design, cfg: Config, dx: float = 2.0,
    ...
    print("  taper ratio            %.2f" % design.taper)
    print("  CL max (3D, built)     %.2f" % s["CLmax_3d"])
    print("  Oswald e               %.3f" % s["oswald_e"])
